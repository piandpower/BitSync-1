...
public class Server {
	static int sPort = 12160;    //The server will be listening on this port number
	...
	public static void main(String args[]) throws IOException {
		...
		while(true){
			...
			System.out.println("There's already a server running on the same port.");
			System.exit(1);
		}
	}
		...
		try{
			...
			for (int i = 0; i < listOfFiles.length; i++) {
				if (listOfFiles[i].isFile()&&listOfFiles[i].getName().toLowerCase().contains("chunk")) {  // 0 1 10 11 12 13 14 2 20 21 22 23 3 4 5 ....
					...
					File file = new File(currentDir+"/src/srcFile/"+chunkName);
					long fileSize=file.length();
					
					Object[] rPayload=f.xPayloadServer(chunkId, fileSize, devId, chunkOwnedArray, filename, in, out);
			...
			f.xPayloadServer("-1", (long)0, devId, chunkOwnedArray, filename, in, out);
			System.out.println("\nAll chunks sent.");
			...
			try{
				...
				out.close();
				csocket.close();
				System.out.println("Client "+devId+" disconnected."+" Thread closed.");
			}
			catch(IOException ioException){
